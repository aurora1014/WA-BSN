...
def wavelets_split(input:torch.Tensor, factor_h:int,factor_w:int,pad:int=0, pad_value:float=0.):
    """
        (n, c, h, w) ===> shuffle(n , c, h/factor , w/factor)
        """
    batch_size, channels, in_height, in_width = input.size()
    out_height = in_height // factor_h
    out_width = in_width // factor_w
    input_view = input.contiguous().view(
        batch_size*channels, out_height, factor_h,
        out_width, factor_w)
    split_out = input_view.permute(2,4, 0, 1, 3).contiguous()
    ff = factor_h * factor_w
    before_shuffle = split_out.view(ff, batch_size * channels, out_height , out_width)
    if pad != 0: before_shuffle =F.pad(before_shuffle, (pad, pad, pad, pad), value=pad_value)
    h=before_shuffle.size(2)
    w=before_shuffle.size(3)
    wavelets_split_out = before_shuffle.view( ff, batch_size*channels,h*w)
    wavelets_split_out1 = wavelets_split_out.permute(1,2,0).contiguous()

    y=F.fold(wavelets_split_out1,output_size=(factor_h*h,factor_w*w),kernel_size=(h,w),stride=(h,w))
    y1=y.reshape(batch_size,channels,factor_h*h,factor_w*w)

    return y1



def wavelets_merge(input:torch.Tensor, factor_h:int,factor_w:int,pad:int=0):
    """
            ( n , c, h/factor, w/factor) ===> (n, c, h, w)
            """
    ff = factor_h * factor_w
    batch_size, channels, in_height, in_width = input.size()
    input=input.reshape(batch_size*channels,1,in_height,in_width)
    h=in_height//factor_h
    w=in_width//factor_w
    wavelets_merge_out=F.unfold(input,kernel_size=(h,w),stride=(h,w))
    before_unshuffle= wavelets_merge_out.permute(2,0,1).contiguous()
    before_unshuffle=before_unshuffle.view( ff, batch_size*channels,h,w)
    if pad != 0: before_unshuffle = before_unshuffle[..., pad:-pad, pad:-pad]
    height=before_unshuffle.size(2)
    width=before_unshuffle.size(3)
    out_height = height * factor_h
    out_width = width * factor_w
    input_view = before_unshuffle.contiguous().view(factor_h, factor_w, batch_size*channels, height, width)
    merge_out = input_view.permute(2, 3,  0, 4, 1).contiguous()
    merge_out = merge_out.view(batch_size, channels, out_height, out_width)
    return merge_out
